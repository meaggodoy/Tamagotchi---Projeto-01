programa
{
	...
	funcao inicio()
	{
		...
		faca {
			...
			se (menu == 1) {
				...
				faca {
					...
					} se (avancarTempo > 2 ou avancarTempo < 1) {
						escreva("Opa, não temos essa opção de menu, digite novamente ;)")
  						escreva("\n--------------")
					}
				...
				faca {
					...
					se (alimentar == 1){
						...
						escreva("\nFome atual: ", fome)
					} se (alimentar > 2 ou alimentar < 1) {
						escreva("Opa, não temos essa opção de menu, digite novamente ;)")
					}
					escreva("\n--------------")
				} enquanto (fome < 10 e felicidade > 0 e alimentar != 2)
			} se (menu == 3) {
				faca {
  				...
  				faca {
					...
					se(tomarBanho == 1){
						...
						escreva(nomePet, " não tomou banho!")
					} se (tomarBanho < 1 ou tomarBanho > 2) {
						escreva("Opa, não temos essa opção de menu, digite novamente ;)")
					}
					escreva("\n--------------")																
  				} enquanto (tomarBanho != 2 e felicidade > 0)
  			} se (menu == 5) {
  				//funcao status -> apresenta cada status do pet ao jogador
  				se (tempoDia <= 1) {
					escreva("\nA idade de ", nomePet, " é ", tempoDia, " dia e ", tempoHora, " horas")
				} se (tempoDia > 1 e tempoDia < 7) {
					escreva("\nA idade de ", nomePet, " é ", tempoDia, " dias e ", tempoHora, " horas")
				} 
		...
		se (menu == 6) {
			escreva("\nPoxa, que pena que você desligou o jogo.. Nos vemos depois!")
		} senao {
			se (felicidade <= 0) {
				felicidade = 0
			...
			se (fome >= 10) {
				...
				escreva("\n", nomePet, " ficou fraquinho demais… A falta de comida o fez perder as forças.\nLembre-se: cuidar é também alimentar.")
			}
			se (limpeza <= 0) {
				limpeza = 0

				escreva("\n", nomePet, " acabou ficando doente por causa da sujeira… Ele tentou aguentar, mas o ambiente ficou pesado demais.\nLimpeza também é uma forma de cuidado.")
			}
			se (tempoDia == 7) {
				escreva("\n", nomePet, " viveu uma vida longa e feliz ao seu lado. Ele se despede com um sorriso, agradecendo por todo o carinho.\nParabéns, você cuidou muito bem dele.")
			}
			escreva("\n--------------\n")
			escreva("Status final: ")
			escreva("Felicidade: ", felicidade, " | Fome: ", fome, " | Limpeza: ", limpeza, " | Tempo de vida: ", tempoDia, " dias e ", tempoHora, " horas")
		}
